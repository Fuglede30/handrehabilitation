...
OUTPUT_DIR = PROJECT_ROOT / "dataset2_class"
...
# Create mapping from part number to class index (based on sorted dimensions)
def create_part_number_to_class_mapping():
    """Map part numbers to class indices based on sorted dimensions."""
    sorted_dimensions = sorted(list(set(PLATE_PART_NUMBERS.values())))
    mapping = {}
    for part_num, dimension in PLATE_PART_NUMBERS.items():
        class_idx = sorted_dimensions.index(dimension)
        mapping[part_num] = class_idx
    return mapping

PART_NUMBER_TO_CLASS_IDX = create_part_number_to_class_mapping()
...
def parse_xml_for_tiles(xml_file, img_file):
    ...
    try:
        ...
        for obj in root.findall('object'):
            ...
            if name not in PLATE_PART_NUMBERS:
                filtered_count += 1
                continue
            ...
            for tile_name, tile_x, tile_y in TILES:
                ...
                if adjusted_bbox:
                    ...
                    class_idx = PART_NUMBER_TO_CLASS_IDX[name]
