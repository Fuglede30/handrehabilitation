...
DATASET_DIR = PROJECT_ROOT / "dataset2_class"  # Multiclass dataset with dimension-based classes
...
def create_dataset_yaml():
    ...
    # Extract and sort dimensions (class names match preprocessing)
    sorted_dimensions = sorted(list(set(PLATE_PART_NUMBERS.values())))
    ...
    dataset_config = {
        ...
        "val": "val/images",
        "test": "test/images",
        "nc": len(sorted_dimensions),  # Number of classes (11 plate types)
        "names": sorted_dimensions  # Class names (dimensions: 2x10, 2x2, 2x3, 2x4, 2x6, 2x8, 3x3, 4x4, 4x6, 4x8, 6x6)
    }
...
def train_yolo():
    ...
    run_dir = Path(getattr(results, "save_dir", OUTPUT_DIR / "train_class"))
    results_csv = run_dir / "results.csv"
    if results_csv.exists():
        saved_metrics = OUTPUT_DIR / "train_class_results.csv"
        shutil.copyfile(results_csv, saved_metrics)
        print(f"Saved per-epoch metrics to: {saved_metrics}")

        plot_path = OUTPUT_DIR / "train_class_loss.png"
...
def main():
    ...
    for split in ["train", "val", "test"]:
        ...
        if not labels_dir.exists():
            ...
            print("Please run preprocess_by_class.py to generate YOLO .txt labels.")
            return
    ...
    try:
        ...
        print(f"Best model: {OUTPUT_DIR / 'train_class' / 'weights' / 'best.pt'}")
        print(f"Last model: {OUTPUT_DIR / 'train_class' / 'weights' / 'last.pt'}")
