...
def main():
    ...
    """
    # Initialize keyboard controller for sending key presses
    keyboard = Controller()
    
    # ===== KEYBINDING CONFIGURATION =====
    # Map square names to keyboard keys
    # Single character: 'a', 'w', 'd', 's', etc.
    # Special keys: 'space', 'up', 'down', 'left', 'right', 'shift', 'ctrl'
    keybindings = {
        "Square1": 'w',           # Forward (W key)
        "Square2": 'a',           # Left (A key)
        "Square3": 's',           # Back (S key)
        "Square4": 'd',           # Right (D key)
        "Square5": 'space',       # Space/Jump
    }
    # ===== END KEYBINDING =====
    
    # Track which squares are currently being touched (per hand)
    # Structure: {hand_no: {square_name: is_touching}}
    touch_state = {}
    ...
    canny_low = 30          # Lower threshold - lower = detect fainter edges
    canny_high = 100        # Upper threshold - lower = detect more edges
    ...
    max_aspect_ratio = 1.7  # Maximum width/height ratio (higher = allow more rectangles)
    
    # NMS (Non-Maximum Suppression) threshold
    nms_iou_threshold = 0.3  # Overlap threshold - higher = remove more overlaps
    # ===== END CONFIGURATION =====
    
    # For FPS calculation
    prev_time = 0
    
    # Track initialization time to auto-detect squares for first 5 seconds
    ...
    while True:
        ...
        if elapsed_time < 0.5 and not detection_complete:
            ...
            print(f"Detection complete! Fixed {len(tracker.clickable_regions)} squares.")
        ...
        if tracker.results.multi_hand_landmarks:
            ...
            for hand_idx in range(len(tracker.results.multi_hand_landmarks)):
                ...
                # Initialize touch state for this hand if not exists
                if hand_idx not in touch_state:
                    touch_state[hand_idx] = {}
                
                # Check all squares for this hand
                for square_name in keybindings.keys():
                    was_touching = touch_state[hand_idx].get(square_name, False)
                    is_touching = square_name in touched_regions
                    
                    # Detect state change
                    if is_touching and not was_touching:
                        # Started touching - press key
                        key = keybindings[square_name]
                        if key == 'space':
                    ...
                    # Update state
                    touch_state[hand_idx][square_name] = is_touching
